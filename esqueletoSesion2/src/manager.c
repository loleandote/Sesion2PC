...
if (signal(SIGINT, manejador_senhal) == SIG_ERR){//si hay un error aparecera un sig_err por lo que le pedimos que nos envie un mensaje.
...
//-------------------------------------------------------------------------------------------------
 ...
 void lanzar_proceso_telefono(const int indice_tabla){
    ...
    g_process_lineas_table[indice_tabla].clase = CLASE_TELEFONO;
